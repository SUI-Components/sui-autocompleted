...
function Autocompleted ({focus,
  ...
  const isFirstSelected = () => active.section === defaultPosition.section &&
      active.suggestion === defaultPosition.suggestion

  const getLastSectionSuggestion = section => withSections ? suggests[section].suggestions.length - 1 : suggests.length - 1
  ...
  const renderResultList = () => (suggests && suggests.length > 0
      ? (
        <ResultsList
          active={active}
